...
// 500 ns latency buckets + remote-op attribution (one block per worker thread).
namespace bench {
ThreadStats g_stats[MAX_APP_THREAD];
}
...
#ifdef COUNT_RDMA
  // Per-thread DSM remote counters are indexed by the registered thread id.
  const int rdma_tid = dsm->getMyThreadID();
#endif
#endif
  ...
  while (counter < thread_op_num) {
...
#ifdef BENCH_LATENCY
    // Snapshot this thread's remote-op counters and start the per-op timer.
    // The delta after the op tells us exactly which remote work it triggered,
    // and whether it was a pure local cache hit on the cached path.
    int lat_op = -1;
#ifdef COUNT_RDMA
    uint64_t r0 = dsm->num_rdma_read[rdma_tid][0];
    ...
    if (lat_op >= 0) {
      ...
      uint64_t ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                        ...
                        .count();
#ifdef COUNT_RDMA
      uint64_t dr = dsm->num_rdma_read[rdma_tid][0] - r0;
      uint64_t dw = dsm->num_rdma_write[rdma_tid][0] - w0;
      uint64_t dc = dsm->num_rdma_cas[rdma_tid][0] - c0;
      uint64_t dp = dsm->num_rdma_rpc[rdma_tid][0] - p0;
      uint64_t dtot = dr + dw + dc + dp;
      int cls = dtot ? bench::CLS_REMOTE : bench::CLS_LOCAL;
      my_stats.record(lat_op, cls, ns);
      my_stats.remote_read += dr;
