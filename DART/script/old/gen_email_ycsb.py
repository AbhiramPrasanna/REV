...
for i, key in enumerate(keys):
    ...
    if now_ml_load_num < ml_load_num and random.random() < ml_load_paritial:
        now_ml_load_num += 1
        result_ml_load += insert_log(key, value)
    elif now_ml_run_num < ml_run_num:
        now_ml_run_num += 1
        result_ml_run += insert_log(key, value)
    elif now_ml_load_num < ml_load_num:
        now_ml_load_num += 1
        result_ml_load += insert_log(key, value)

result = load_predata(now_total_num) + result
result_ml_load = load_predata(now_ml_load_num) + result_ml_load
result_ml_run = load_predata(now_ml_run_num) + result_ml_run
with open(load_file, "w") as load:
    load.write(result)
with open(load_small_insert_file, "w") as load:
    load.write(result_ml_load)
with open(run_big_insert_file, "w") as load:
    load.write(result_ml_run)
input_f.close()
print(f"{now_total_num = }, {new_d_insert_num = }, {now_ml_load_num = }, {now_ml_run_num = }")
...
for num, i in enumerate(indices):
    if num % 100_000 == 0:
        print(f"md_run = {num}")
    if now_insert < insert_num and random.random() < run_write_partial_d:
        result += insert_log(new_keys[now_insert], data_str("n_", new_keys[now_insert]))
        now_insert += 1
    elif now_search < search_num:
        result += search_log(keys[i])
        now_search += 1
    elif now_insert < insert_num:
        result += insert_log(new_keys[now_insert], data_str("n_", new_keys[now_insert]))
        now_insert += 1
result = load_predata(total_num) + result
