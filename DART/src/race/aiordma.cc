...
rdma_dev::rdma_dev(/*const char *dev_name,*/int dev_index, int _ib_port, int _gid_idx)
    : ib_port(_ib_port), gid_idx(_gid_idx), conn_ids(65536)
{
    ...
    {
        race_log_warn("failed to query device attributes");
    ...
    race_log_info("rdma_dev init success");
}
...
rdma_dev::~rdma_dev()
{
    ...
    if (pd && ibv_dealloc_pd(pd))
        race_log_warn("failed to dealloc pd");
    if (ib_ctx && ibv_close_device(ib_ctx))
        race_log_warn("failed to close device");
#ifdef ENABLE_DOCA_DMA
    if (dma_dev && doca_dev_close(dma_dev) != DOCA_SUCCESS)
        race_log_warn("failed to close dma dev");
#endif
    race_log_info("rdma_dev destory success");
}
...
rdma_worker::~rdma_worker()
{
    if (cq && ibv_destroy_cq(cq))
        race_log_warn("failed to destory cq");
    if (mpmr)
...
void rdma_worker::worker_loop()
{
    ...
    while (loop_flag)
    {
        ...
        {
            auto &&wc = wcs[i];
            race_log_debug("polled %lu", wc.wr_id);

            if(wc.wr_id == wr_wo_await){
                if (wc.status != IBV_WC_SUCCESS)
                {
                    race_log_warn("got bad completion with status: 0x%x, vendor syndrome: 0x%x", wc.status, wc.vendor_err);
                }
                continue;
            }
            ...
            if (cor->coro_state & coro_state_inited)
            {
                race_log_debug("imm resume");
                cor->resume_handler();
            }
            else
            {
                race_log_debug("wait for resume");
            ...
            if (select_list.select())
            {
                ...
                for (size_t i = 0; i < sk_num; ++i)
                {
                    ...
                    if (sk == listenfd)
                    {
                        ...
                        race_log_info("accept conn: %d", accepted_sock);
                    ...
                    else
                    {
                        ...
                        if (recv_len <= 0)
                        {
                            ...
                            close(sk);
                            select_list.del(sk);
                            race_log_info("connection closed: %d", sk);
                        }
                        else
                        {
...
rdma_conn::~rdma_conn()
{
    ...
    if (qp && ibv_destroy_qp(qp))
        race_log_warn("failed to destroy qp");
    if (exchange_mr)
        rdma_free_mr(exchange_mr);
    release_working_coros();
    dev.free_conn_id(conn_id);
    race_log_info("close conn %d  %u", sock, conn_id);
}
...
int rdma_conn::modify_qp_to_init()
{
    ...
    if ((rc = ibv_modify_qp(qp, &attr, flags)))
        race_log_err("failed to modify QP state to INIT");
    return rc;
}
...
int rdma_conn::modify_qp_to_rtr(uint32_t rqp_num, uint16_t rlid, const ibv_gid *rgid)
{
    ...
    if ((rc = ibv_modify_qp(qp, &attr, flags)))
        race_log_err("failed to modify QP state to RTR");
    return rc;
}
...
int rdma_conn::modify_qp_to_rts()
{
    ...
    if ((rc = ibv_modify_qp(qp, &attr, flags)))
        race_log_err("failed to modify QP state to RTS");
    return rc;
}
...
task<> rdma_conn::update_cache()
{
    ...
    for (uint64_t i = 0; i < hdr->tail;)
    {
        ...
        else if (info->type == rdma_info_type_mmap)
        {
            ...
            race_log_err("unknown info type");
    }
}
...
void dma_future::await_resume()
{
    if (src_buf && doca_buf_refcount_rm(src_buf, nullptr) != DOCA_SUCCESS)
        race_log_warn("failed to free src buf");
    if (dst_buf && doca_buf_refcount_rm(dst_buf, nullptr) != DOCA_SUCCESS)
        race_log_warn("failed to free dst buf");
}
...
void free_mmap(doca_mmap *mmp)
{
    if (doca_mmap_destroy(mmp) != DOCA_SUCCESS)
        race_log_warn("failed to destroy mmap");
}
