...
void Server::Init(Directory *dir)
{
    ...
    Segment *tmp;
    race_log_err("init depth: %lu", INIT_DEPTH);
    for (uint64_t i = 0; i < dir_size; i++)
    {
                       ...
                       uint64_t _cli_id, uint64_t _coro_id)
{
    ...
    // alloc info
    alloc.Set((char *)lmr->addr, lmr->length);
    race_log_info("laddr:%lx llen:%lx", (uint64_t)lmr->addr, lmr->length);
    rmr = cli->run(conn->query_remote_mr(233));
    race_log_info("raddr:%lx rlen:%lx rend:%lx", (uint64_t)rmr.raddr, rmr.rlen, rmr.raddr + rmr.rlen);
...
task<> Client::start(uint64_t total)
{
    ...
    *start_cnt = 0;
    ...
    // race_log_err("Start_cnt:%lu", *start_cnt);
...
task<> Client::cal_utilization(){
    ...
    race_log_err("global_dep:%lu",dir->global_depth);
    ...
    for(uint64_t i = 0 ; i < (1<<dir->global_depth) ; i++){
        ...
        // race_log_err("seg:%lu first_index:%lu dir->segs[i].local_depth:%lu flag:%d",i,first_index,dir->segs[i].local_depth,dir->segs[i].local_depth == dir->global_depth || i == first_index);
...
Retry:
    // race_log_err("[%lu:%lu:%lu] op_key:%lu",machine_id,cli_id,coro_id,this->op_key);
...
#else
    auto rbuc1 = conn->read(bucptr_1, rmr.rkey, buc_data, sizeof(Bucket), lmr->lkey);
#endif
...
// #endif
    ...
    {
        // co_await sync_dir();
        race_log_err("[%lu:%lu:%lu] op_key:%lu segloc:%lu buc_data->local_depth:%u (buc_data + 2)->local_depth:%u dir->segs[segloc].local_depth:%lu",machine_id,cli_id,coro_id,this->op_key,segloc,buc_data->local_depth,(buc_data + 2)->local_depth,dir->segs[segloc].local_depth);
        // goto Retry;
        co_return;
    }
    ...
    uint64_t buc_idx = bucidx_1;
    Bucket *buc = buc_data;
    uintptr_t buc_ptr = bucptr_1;
    uintptr_t slot_ptr = FindEmptySlot(buc, buc_idx, buc_ptr);

    if (slot_ptr == 0ul)
    {
        race_log_err("[%lu:%lu]%s split for key:%lu with local_depth:%u global_depth:%lu at segloc:%lx",cli_id,coro_id,(buc_data->local_depth==dir->global_depth)?"gloabl":"local",*(uint64_t*)key->data,buc_data->local_depth,dir->global_depth,segloc);
        co_return;
    ...
    tmp->pos = node.pos;
    tmp->type = node.type;
    tmp->offset = node.fptr;
    // tmp->len = kvblock_len;
    // tmp->offset = ralloc.offset(kvblock_ptr);
    if (!co_await conn->cas_n(slot_ptr, rmr.rkey, 0, *(uint64_t *)tmp))
    {
        // race_log_err("[%lu:%lu:%lu] op_key:%lu segloc:%lu fail to cas at slot_ptr:%lx",machine_id,cli_id,coro_id,this->op_key,segloc,slot_ptr);
        goto Retry;
    }
    ...
    //             {
    //                 // race_log_err("[%lu:%lu]Duplicate-key :%lu", cli_id, coro_id, *(uint64_t *)key->data);
    ...
    // {
    //     // race_log_err("[%lu:%lu:%lu] op_key:%lu segloc:%lu wrong bucket",machine_id,cli_id,coro_id,this->op_key,segloc);
...
task<int> Client::Split(uint64_t seg_loc, uintptr_t seg_ptr, uint64_t local_depth, bool global_flag)
{
    ...
    // if (global_flag)
    // {
    //     race_log_err("[%lu:%lu:%lu] %lu update global_depth from:%lu to %lu",machine_id,cli_id,coro_id,this->op_key,dir->global_depth,dir->global_depth+1);
...
//                     race_log_err("[%lu:%lu:%lu]Fail to move slot_ptr:%lx",machine_id,cli_id,coro_id,slot_ptr);
//                     continue;
//                 }
...
task<std::tuple<uintptr_t, uint64_t>> Client::search(Slice *key, Slice *value)
{
    ...
    {
        // race_log_err("Locked Segment After Load");
    ...
    if (IsCorrectBucket(segloc, buc_data, pattern_1) == false)
    {
        // race_log_err("Wrong Bucket After Load");
        auto slot_info = co_await search_on_resize(key, value);
        perf.push_search();
        co_return slot_info;
    }
    ...
    // race_log_err("[%lu:%lu]No match key :%lu", cli_id, coro_id, *(uint64_t *)key->data);
    perf.push_search();
    sum_cost.push_level_cnt(1);
...
Retry:
    if((++cnt)%1000==0)
    {
        race_log_err("[%lu:%lu]search_on_resize for key:%lx",cli_id,coro_id,*(uint64_t*)key->data);
        co_return std::make_tuple(slot_ptr, slot);
    }
    alloc.ReSet(sizeof(Directory));
    co_await sync_dir();
...
#else
    for (uint64_t round = 0; round < 1; round++)
#endif
    {
        buc = buc_data + round;
        buc_ptr = bucptr_1;
        ...
        for (uint64_t i = 0; i < SLOT_PER_BUCKET; i++)
        {
            if (*(uint64_t*)(&buc->slots[i]) && buc->slots[i].fp == fp(pattern_1))
            {
                ...
                // }
                Node_Meta node;
                node.pos = buc->slots[i].pos;
                node.type = buc->slots[i].type;
                node.fptr = buc->slots[i].offset;
                memcpy(value->data + value->len, &node, sizeof(uint64_t));
...
//     {
//         // if((++cnt)%1000==0)
//         //     race_log_err("[%lu:%lu]slot_ptr:%lx slot:%lx for %lu to be deleted with pattern_1:%lx",cli_id,coro_id,slot_ptr,slot,*(uint64_t*)key->data,pattern_1);
//         // 3rd RTT: Setting the key-value block to full zero
//         if (!co_await conn->cas_n(slot_ptr, rmr.rkey, slot , 0)){
//             race_log_err("[%lu:%lu]fail to cas slot_ptr:%lx for %lu to zero",cli_id,coro_id,slot_ptr,*(uint64_t*)key->data);
//             goto Retry;
//         }
//     }else{
//         race_log_err("[%lu:%lu]No match key for %lu to be deleted",cli_id,coro_id,*(uint64_t*)key->data);
//     }
    co_return;
}
...
//     if (slot_ptr != 0ull)
//     {
//         // race_log_err("[%lu:%lu]slot_ptr:%lx slot:%lx for %lu to be updated with new slot: fp:%d len:%d offset:%lx",cli_id,coro_id,slot_ptr,slot,*(uint64_t*)key->data,tmp->fp,tmp->len,tmp->offset);
//         if(cnt++==0) co_await std::move(wkv) ;
//         // 3rd RTT: Setting the key-value block to full zero
//         if (!co_await conn->cas_n(slot_ptr, rmr.rkey, slot, *(uint64_t*)tmp))
//             goto Retry;
//     }else{
//         race_log_err("[%lu:%lu]No match key for %lu to update",cli_id,coro_id,*(uint64_t*)key->data);
//     }
    co_return;
}
