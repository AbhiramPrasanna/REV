...
#define race_log_err(fmt, arg...) (do_log(ERROR, fmt, ##arg), exit(0))
#else
#define race_log_err(fmt, arg...) do_log(ERROR, fmt, ##arg)
#endif
#else
#define race_log_err(fmt, arg...)
#endif
#if LOG_LEVEL >= LOG_LEVEL_WARNING
#define race_log_warn(fmt, arg...) do_log(WARNING, fmt, ##arg)
#else
#define race_log_warn(fmt, arg...)
#endif
#if LOG_LEVEL >= LOG_LEVEL_INFO
#define race_log_info(fmt, arg...) do_log(INFO, fmt, ##arg)
#else
#define race_log_info(fmt, arg...)
#endif
#if LOG_LEVEL >= LOG_LEVEL_DEBUG
#define log_debug(fmt, arg...) do_log(DEBUG, fmt, ##arg)
#else
#define race_log_debug(fmt, arg...)
#endif
...
inline void *alloc_hugepage(size_t size, int page_flags = hugepage_2m_flags) {
    ...
    {
        race_log_warn("failed to map hugepages: %s", strerror(errno));
        return nullptr;
    }
#else
    if (posix_memalign(&res, 1 << (page_flags >> MAP_HUGE_SHIFT), size)) [[unlikely]]
    {
        race_log_warn("failed to malloc: %s", strerror(errno));
        return nullptr;
    }
#endif
    return res;
...
inline void free_hugepage(void *ptr, size_t size) {
    ...
    { race_log_warn("failed to unmap hugepages: %s", strerror(errno)); }
#else
    free(ptr);
#endif
}
...
public:
    ...
    void free(void *ptr) {
        ...
        if (elem->flag != flag_inuse) {
            race_log_warn("invaild ptr: head: %u  tail: %u  cap: %u", head, tail, blk_cap);
            return;
        }
        elem->flag = flag_gc;
