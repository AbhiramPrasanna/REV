...
constexpr uint64_t INIT_DEPTH = 15;
...
struct Directory
{
    ...
    void print(){
        race_log_err("Global_Depth:%lu",global_depth);
        for(uint64_t i = 0 ; i < (1<<global_depth) ; i++){
            race_log_err("Entry %lu : local_depth:%lu seg_ptr:%lx ",i,segs[i].local_depth,segs[i].seg_ptr);
        }
    }
} __attribute__((aligned(1)));
...
{
  public:
    Server(int dev_index, Config &config);
    ~Server();
