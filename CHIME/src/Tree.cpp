...
static inline bool should_offload(int tid) {
  static thread_local int ctr = 0;
  if (g_offload_rate >= 100) return true;
  if (g_offload_rate <= 0) return false;
  int slot = ctr++ % 100;
...
#ifdef ENABLE_OFFLOAD
GlobalAddress Tree::get_leaf_addr(const Key &k, CoroPull* sink) {
  // Same descent as Tree::search, but stop at the leaf's parent and return the
  // covering leaf address (level 1 => p points to a leaf).
  bool from_cache = false;
  const TreeCacheEntry *cache_entry = nullptr;
  GlobalAddress p, sibling_p;
  uint16_t level;
...
int Tree::range_query_offload(const Key &from, const Key &to, std::map<Key, Value> &ret) {  // [from, to)
  ...
  while (cur < to) {
    ...
    if (cnt > 0) {
      dsm->read_sync(buffer, result_addr, cnt * sizeof(std::pair<Key, Value>), nullptr);
      auto *kvs = (std::pair<Key, Value> *)buffer;
      for (int i = 0; i < cnt; ++i) ret[kvs[i].first] = kvs[i].second;
      total += cnt;
      offload_scan_kv[tid] += cnt;
    }
    // Done if the last leaf visited already reaches `to`, produced nothing, or
    // we filled a batch shy of the cap (range exhausted on this node).
    if (max_key == define::kkeyNull || !(max_key < to)) break;
    if (cnt < chime_offload::kScanSlotCap) break;
    cur = max_key + (uint8_t)1;  // resume just past the last key returned
  }
  return total;
}
