...
ibv_mr *createMemoryRegionOnChip(uint64_t mm, uint64_t mmSize,
                                 RdmaContext *ctx) {
  // rdma-core has no experimental on-chip device memory (ibv_exp_alloc_dm).
  // Fall back to a regular DRAM memory region -- the lock pool only needs
  // REMOTE_ATOMIC/READ/WRITE access, which a normal MR provides. (On-chip was
  // a latency optimization for the HOCL lock, gated by TREE_TEST_HOCL_HANDOVER,
  // which is off by default.)
  memset((void *)mm, 0, mmSize);
  return createMemoryRegion(mm, mmSize, ctx);
}
...
bool createQueuePair(ibv_qp **qp, ibv_qp_type mode, ibv_cq *send_cq,
  ...
  // Standard rdma-core QP. The exp version requested extended atomic args
  // (max_atomic_arg) for masked atomics; those are dropped, and a plain RC QP
  // already supports the 8-byte CAS/FAA CHIME uses.
...
// createDCTarget removed: DC transport was ibv_exp only and unused by CHIME.
